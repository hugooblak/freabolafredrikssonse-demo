...
add_filter(
	...
	function ( $exporters ) {
		$exporters['freabolafredrikssonse-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'freabolafredrikssonse-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'freabolafredrikssonse-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'freabolafredrikssonse-leads',
						'group_label' => __( 'Quote requests', 'freabolafredrikssonse-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['freabolafredrikssonse-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'freabolafredrikssonse-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
