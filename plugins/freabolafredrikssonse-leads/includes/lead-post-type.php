...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'freabolafredrikssonse-leads' ),
					'singular_name' => __( 'Lead', 'freabolafredrikssonse-leads' ),
					'edit_item'     => __( 'Förfrågan', 'freabolafredrikssonse-leads' ),
					'search_items'  => __( 'Search leads', 'freabolafredrikssonse-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'freabolafredrikssonse-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'freabolafredrikssonse-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'freabolafredrikssonse-leads' ),
			'npl_phone'    => __( 'Phone', 'freabolafredrikssonse-leads' ),
			'npl_service'  => __( 'Needs', 'freabolafredrikssonse-leads' ),
			'npl_timeline' => __( 'When', 'freabolafredrikssonse-leads' ),
			'npl_zip'      => __( 'ZIP', 'freabolafredrikssonse-leads' ),
			'npl_source'   => __( 'Source', 'freabolafredrikssonse-leads' ),
			'npl_status'   => __( 'Status', 'freabolafredrikssonse-leads' ),
			'date'         => __( 'Received', 'freabolafredrikssonse-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'freabolafredrikssonse-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'freabolafredrikssonse-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'freabolafredrikssonse-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'freabolafredrikssonse-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'freabolafredrikssonse-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'freabolafredrikssonse-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'freabolafredrikssonse-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'freabolafredrikssonse-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'freabolafredrikssonse-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'freabolafredrikssonse-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'freabolafredrikssonse-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'freabolafredrikssonse-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
