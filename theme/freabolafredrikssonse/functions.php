<?php
/**
 * FREAB, Ola Fredrikssons Elektriska AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "FREAB, Ola Fredrikssons Elektriska AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through fr_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'freabolafredrikssonse', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'freabolafredrikssonse' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'freabolafredrikssonse' ) ) );

		register_block_pattern_category( 'freabolafredrikssonse-sections', array( 'label' => __( 'FREAB, Ola Fredrikssons Elektriska AB sections', 'freabolafredrikssonse' ) ) );
		register_block_pattern_category( 'freabolafredrikssonse-pages', array( 'label' => __( 'FREAB, Ola Fredrikssons Elektriska AB page layouts', 'freabolafredrikssonse' ) ) );
	}
);
