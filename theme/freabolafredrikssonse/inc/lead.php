 ...
 * or photo. They call fr_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional FREAB, Ola Fredrikssons Elektriska AB.
 *
 ...
 * @return string
 */
function fr_sample_note() {
	return fr_lead( 'ar_riktig' ) ? '' : ' <span class="fr-sample">(platshållare)</span>';
}
 ...
 * @return string File name for fr_lead_img().
 */
function fr_lead_bild( $i ) {
	$bilder = (array) fr_lead( 'bilder', array() );
 ...
 * @return string
 */
function fr_ring_label() {
	$namn = fr_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, fr_lead( 'telefon' ) ) : 'Ring ' . fr_lead( 'telefon' );
}
